# Assumption: cluster number equals to component id
    ...
    def createSymbolTable(self, constTable):
        ...
        for dataDec in dataDecList.getChildren():
            ...
            if dataType.getChild(
                    0).getText() != 'iterator' and dataType.ASSIGN() is None:  # means non_iterator (like model_input)
...
class DotGenerator:
    ...
    def gendot_graph(self, dfg):
        ...
        dotcode = self.subgraph_templ.format(str(dfg.id),
                                             dfg.name,
                                             subgraph_code)
        return dotcode
    ...
    def genedges(self, dfg):
        ...
        for child in dfg.state_flows.keys():
            edge = dfg.state_flows[child]
            edgelabel = edge.name if edge is not None else None
            cmp = self.genlabel(dfg)

            flow = str.format('{} -> {} [label="{}"];\n', cmp, cmp, edgelabel)

            edge_code += flow

        for child in dfg.output_flows.keys():
            ltail = ''
            lhead = ''
            cluster_connection = ''

            edge = dfg.output_flows[child]
            left = self.genlabel(edge.output_component)
            right = self.genlabel(edge.input_component)

            if not edge.output_component.primitive:
                o_logical_comp = self.find_last_output(child, edge.output_component.sub_components)
                if not o_logical_comp:
                    print "Could not find matching edge"
                lhead = "lhead=cluster_" + str(edge.output_component.id)
                left = self.genlabel(o_logical_comp)

            if not edge.input_component.primitive:
                i_logical_comp = self.find_first_input(child, edge.input_component.sub_components)
                if not i_logical_comp:
                    print "Could not find matching edge"
                ltail = "ltail=cluster_" + str(edge.input_component.id)
                right = self.genlabel(i_logical_comp)



            if ltail or lhead:
                cluster_connection = '[' + ltail + ' ' + lhead + ']'

            edgelabel = edge.name if edge is not None else None

            flow = str.format('{} -> {} [label="{}"] {};\n', left, right, edgelabel, cluster_connection)
            edge_code += flow

        return edge_code

    def find_last_output(self, name, components):

        for component in reversed(components):
            if name in component.output_flows.keys() or name in component.state_flows.keys():
                return component

        return None

    def find_first_input(self, name, components):

        for component in components:
            if name in component.input_flows.keys() or name in component.state_flows.keys():
                return component

        return None

    def genlabel(self, graph):
        label = '{"' + str(graph.id) + '" [label="' + graph.name + '"]' + '}'
        return label
