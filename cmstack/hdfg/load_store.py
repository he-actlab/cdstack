...
from cmstack.hdfg.hdfg_pb2 import *
...
def _get_all_tensors(cmstack_program_proto):  # type: (Program) -> Iterable[Tensor]
    """Scan an ONNX program for all tensors and return as an iterator."""
    return chain(_get_initializer_tensors(cmstack_program_proto),
                 _get_attribute_tensors(cmstack_program_proto))


def _get_initializer_tensors(cmstack_program_proto):  # type: (Program) -> Iterable[Tensor]
    """Create an iterator of initializer tensors from ONNX program."""
    for initializer in cmstack_program_proto.graph.initializer:
        yield initializer


def _get_attribute_tensors(cmstack_program_proto):  # type: (Program) -> Iterable[Tensor]
    """Create an iterator of tensors from node attributes of an ONNX program."""
    for node in cmstack_program_proto.graph.sub_graph:
