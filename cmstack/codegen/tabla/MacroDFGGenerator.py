...
class MacroDFGGenerator:
    ...
    def create(self, tree):
        ...
        print('done with stats...going to SGD')
        # Append SGD
        for g in self.linkTable:
            ...
            for i in range(len(iterList)):
                if not iterList[i].isdigit():
                    print(self.constTable[iterList[i]])
            ...
            if len(iterList) is 0:
                ...
                for i in range(iterList[0]):
                    print('i val: {}'.format(i))
                    if len(iterList) is 1:
                        gSym = g[0:g.find('[')] + '[' + str(i) + ']'
                        print('gsym is: {}'.format(gSym))
                        if gSym in self.symTable:
                            #print('symtable gsym: {}'.format(self.symTable[gSym]))
                            mult = DFGNode()
                            mult.operation = "*"
                            #mult.dataType = 'gradient'
                            mult.dataType = None
    ...
    def funcTraversal(self, currTerm0, iterDict):
        ...
        if numParents == 1 :
            ...
            a= self.funcMultiParentTraversal(currTerm0, iterDict)
            #print('func: {}'.format(a))
            return a
    ...
    def funcMultiParentTraversal(self, currTerm0, iterDict):
        ...
        operator = '+' if (functionType == 'sum' or functionType == 'norm') else (
                   ...
                   '')
        #print('func val: {}'.format(funcVals))
        ...
        #print('func type: {}'.format(functionType))
        while len(funcVals) > 1:
            funcVals_tmp = []
        ...
        # quick hack
        funcVals[0].operation = functionType
