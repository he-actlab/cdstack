...
import json
import math
from include.TablaLexer import TablaLexer
from include.TablaParser import TablaParser
...
import weight
from MacroDFGGenerator import MacroDFGGenerator
...
def main():
    ...
    input_file = FileStream(dir_path  + "/benchmarks/recommender.t")
    ...
    # only to test macro dfg generation
    if not os.path.exists(dir_path + '/artifacts/'):
        os.makedirs(dir_path  +  '/artifacts/')
    macro_dfg_file = dir_path  + '/artifacts/macro_dfg.json'
    macro_dfg_generator = DFGGenerator()
    macro_dfg = macro_dfg_generator.create(tree)
    macro_dfg_generator.writeTo(macro_dfg, macro_dfg_file)
    sched = Scheduler()
    macro_schedule = sched.createSchedule(macro_dfg, config['num_pes'])
    sched.writeTo(dir_path + '/artifacts/macro_sched.json')
    dotgen = DotGenerator()
    macro_dotcode = dotgen.generateDot(macro_dfg, dotgen.readSched(dir_path + '/artifacts/macro_sched.json'))
    dotgen.writeTo(macro_dotcode, dir_path + '/artifacts/macro_dot.dot')


    # graph creation should happen here
    #print('\n\n================================')

    dfg_file = dir_path + '/artifacts/dfg.json'
    ...
    #dfg = inst.readFrom(dir_path + "/artifacts/nodes_ir.json")
