...
class DotGenerator:
    ...
    def generateDot(self, importedDFG, cycle2id):
        ...
        idDict = {}
        idDict[dfg.get(0)] = '{"source" [label=< <B> source </B> > fixedsize=true]}'
        idDict[dfg.get(1)] = '{"sink" [label=< <B> sink </B> > fixedsize=true]}'
    ...
    def getLabel(self, node):
        ...
        if node.operation == '>':
            node.operation = '&gt;'
        if color is None:
            return '{"' + str(node.id) + '" [label=< <B> ' + node.operation + \
                ' </B> > fontsize=23 fixedsize=false]' + '}'
        else:
            return '{"' + str(node.id) + '" [label=< <B> ' + \
                node.operation + ' </B> > style=filled fillcolor="' + color + \
                '" fontsize=23 fixedsize=false]' + '}'
